...
本文档用于定义 AgentSkeleton 的第一个官方 Codex 接入产物。

对于 `v0.1.0`，推荐的 Codex 接入形态是一个随仓库分发的 skill，用来教会 Codex 如何提供 AgentSkeleton 的零命令工作流体验。
...
- 让 Codex 可以立即复用共享 host adapter 规范

这并不排斥未来继续演进成 plugin。

它只是意味着：第一版官方 Codex 接入应优先追求清晰、对齐和验证速度。
...
- 仓库随代码一起分发 Codex skill
- 当任务适合 AgentSkeleton 时，Codex 加载这个 skill
- 用户仍然只停留在自然对话中
- AgentSkeleton 命令隐藏在宿主层背后

## `v0.1.0` 成功标准

当满足以下条件时，Codex 接入可以被视为满足 `v0.1.0` 的最小要求：

- Codex 无需用户选命令即可激活 AgentSkeleton
- Codex 可以初始化或恢复工作流状态
- Codex 能在澄清与起草之间稳定路由
- Codex 能通过 `response --apply` 写回被接受的结构化结果
- Codex 能从 `post_apply_plan` 继续推进
- Codex 能刷新过期 drafting package
...
3. 在 skill 路径被证明有效后，再决定 Codex 是否还需要 plugin 或 wrapper 形态

## 结论

对 `v0.1.0` 来说，官方 Codex 接入应保持足够轻。
